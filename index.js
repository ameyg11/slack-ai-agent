...
class SlackAIAgent {
    ...
    this.slack.event("member_joined_channel", async ({ event }) => {
      try {
        if (event.channel_type === "C") {
          // Only analyze if it's a public channel
  ...
  setupExpress() {
    ...
    this.app.get("/test/slack-channel", async (req, res) => {
      try {
      ...
      this.app.post("/test/analyze-member", async (req, res) => {
        try {
          ...
          if (!memberInfo) {
            return res
              .status(400)
              .json({ status: "error", message: "memberInfo is required" });
          }
  ...
  async getGitHubInfo(name) {
    try {
      const response = await axios.get(
        `http://api.github.com/search/users?q=${encodeURIComponent(name)}`,
        { timeout: 5000 },
      );
      ...
      if (response.data.items && response.data.items.length > 0) {
        ...
        return {
          ...
          content: `GitHub: ${user.public_repos} public repositories`,
          type: "github",
        };
      }
    } catch (error) {
  ...
  async analyzeWithAI(memberInfo, researchData) {
    ...
    try {
      ...
      return {
        fitScore: 50,
        insights: ["Unable to complate full analysis"],
  ...
  async postAnalysisToChannel(member, analysis, researchData) {
    ...
    console.log("Posting to channel:", process.env.SLACK_PRIVATE_CHANNEL_ID);

    try {
      const joinResult = await this.WebClient.conversations.join({
        channel: process.env.SLACK_PRIVATE_CHANNEL_ID,
      });

      console.log("JOIN RESULT:", joinResult);
    } catch (err) {
      console.log("JOIN ERROR:", err.data || err.message);
    }
  ...
  async start() {
    try {
      console.log(await this.WebClient.auth.test());
